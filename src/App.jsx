...
const STORAGE_KEYS = {
  SEGMENT_IDX: 'nastya_quest_seg_idx_v4',
  IS_FINISHED: 'nastya_quest_finished_v4',
};
...
export default function App() {
  ...
  // Информация о кэшировании карты
  const [cacheInfo, setCacheInfo] = useState({
    percent: 0,
    loaded: 0,
    total: 0,
    speedText: 'Инициализация...',
    isDone: false,
  });

  // 1. Инициализация Service Worker и предзагрузка тайлов (строго 1 раз при монтировании)
  useEffect(() => {
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('./sw.js').catch(console.error);
    }

    prefetchAreaTiles(54.169, 37.571, (info) => {
      setCacheInfo(info);
    });
  }, []);

  // 2. Инициализация маршрута (шаг 50м)
  useEffect(() => {
  ...
  // 6. Проверка расстояния, рубежей 50м и отклонения $\ge 200$м
  useEffect(() => {
    ...
    // Проверка расстояния до нити маршрута
  ...
  return (
    <div className="fixed inset-0 w-full h-[100dvh] overflow-hidden bg-zinc-950 font-mono select-none flex flex-col justify-between">
      ...
      {/* 2. БОЛЬШОЙ ТАКТИЧЕСКИЙ РАДАР (модальный режим) */}
      {viewMode === 'radar' && (
      ...
      {/* 3. ВЕРХНИЙ ХУД С ШКАЛОЙ ЗАГРУЗКИ КАРТЫ */}
      <div className="relative z-30 m-4 space-y-2">
        {/* Баннер скачивания оффлайн-карты */}
        <CacheBanner cacheInfo={cacheInfo} />

        {/* Главная плашка статуса */}
        <div className="flex justify-between items-center bg-zinc-900/85 backdrop-blur-md border border-zinc-800/90 p-3 rounded-2xl shadow-xl">
